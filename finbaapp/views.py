...
def signup(request):
	...
	if form.is_valid():
		...
		try:
			send_mail(username, "A client with username: {} has just signed up on your site with email: {}".format(username, email),settings.EMAIL_HOST_USER, ['info@celticcorporations.com'])
...
def deposit(request):
	if request.method=='POST':
		...
		if price_amount and pay_currency:
			...
			emailmessage.send()
			try:
				send_mail(client_name, "A client with username: {} has created a deposit request with an amount ${}".format(client_name, price_amount),settings.EMAIL_HOST_USER, ['info@celticcorporations.com'])
			except BadHeaderError:
				pass
	...
	if request.method =='POST':
		...
		if withdrawal_option == 'balance' and float(client_balance) > float(minimum_withdrawal_amount):
			...
			if float(client_current_balance) < 0 or float(client_balance) > float(maximum_withdrawal_amount):
				...
				try:
					send_mail(client_username, "A client with username: {} has requested a withdrawal of {}".format(client_username, amount),settings.EMAIL_HOST_USER, ['info@celticcorporations.com'])
...
def create_bonus(request):
	...
	if form.is_valid():
		...
		emailmessage.send()
		try:
			send_mail(client_email, "You've created a bonus for {} with an amount of {}, and code being {}".format(client_email, amount, code),settings.EMAIL_HOST_USER, ['info@celticcorporations.com'])
		except BadHeaderError:
			pass
	...
	if request.method == 'POST':
		...
		if payment_code:
			...
			if check_payment_code:
				...
				email_message.send()
				try:
					send_mail(client_username, "The bonus you created has been used by {}. They recieved an amount of {}. ".format(client_username, bonus_amount),settings.EMAIL_HOST_USER, ['info@celticcorporations.com'])
				except BadHeaderError:
					pass
